...
"""
TODO - right now, the arguments to this function are still file paths. But, they could also be DataLoader
objects - this would allow us to load eventlog files from the their source for e.g. some preliminary validation,
but not actually fully "parse" them until we get into here.

It does seem to me that most of the time, the 
spark_eventlog_path that has been passed in is a tempfile directory - a tempfile directory allows us to achieve this
as well, because data is loaded once into that tmp directory, and then can be read many times by many different places
However, this is not well-encapsulated because we have to inspect these paths in order to figure out what type of
destination we are reading from (i.e. local or remote)
"""
