...
@pytest.mark.parametrize("event_log_url", ["s3://sync-test-artifacts/airlinedelay/jb-42K1E16/emr.zip", "s3://sync-test-artifacts/airlinedelay/jb-42K1E16/"])
@pytest.mark.parametrize("event_log_file_archive,event_log_s3_dir", [(Path("tests", "logs", "emr.zip"), "airlinedelay/jb-42K1E16/")])
def test_emr_log_from_s3(event_log_url, event_log_file_archive, event_log_s3_dir):
    ...
    stubber.add_response("list_objects_v2", {"Contents": [{"Key": s3_key, "Size": s3_object_size}]}, {"Bucket": s3_bucket, "Prefix": s3_prefix})
    stubber.add_response("head_object", {"ContentLength": s3_object_size, "ContentType": "application/octet-stream"}, {"Bucket": s3_bucket, "Key": s3_key})
    ...
    with open(event_log_file_archive, "rb") as fobj:
        ...
        while chunk:
            ...
            stubber.add_response("get_object", {"ContentLength": len(chunk), "ContentType": "application/octet-stream", "Body": io.BytesIO(chunk)}, expected_params)
...
@pytest.mark.parametrize("event_log_url", ["s3://sync-test-artifacts/airlinedelay/jb-42K1E16/", "s3://sync-test-artifacts/airlinedelay"])
@pytest.mark.parametrize("event_log_file_archive,event_log_s3_dir", [(Path("tests", "logs", "databricks-rollover-messy.zip"), "airlinedelay/jb-42K1E16/")])
def test_databricks_log_from_s3_dir(event_log_url, event_log_file_archive, event_log_s3_dir):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with ZipFile(event_log_file_archive) as zfile:
            contents = [(event_log_s3_dir + "/".join(zinfo.filename.split(os.sep)), zinfo) for zinfo in zfile.infolist() if not zinfo.is_dir()]
            stubber.add_response("list_objects_v2", {"Contents": [{"Key": key, "Size": zinfo.file_size} for key, zinfo in contents]}, {"Bucket": s3_bucket, "Prefix": s3_prefix})

            for s3_key, zinfo in contents:
                with zfile.open(zinfo) as zobj:
                    stubber.add_response("head_object", {"ContentLength": zinfo.file_size, "ContentType": "application/octet-stream"}, {"Bucket": s3_bucket, "Key": s3_key})
